...
export default function QuoteManagement({
      ...
      <div className="space-y-4">
        ...
        {quotePrice && (
          <div>
            ...
            <span className="ml-2 text-lg font-bold text-green-600">${quotePrice.toFixed(2)}</span>
          </div>
        )}
      </div>
    );
  ...
  const handleStatusChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    await updateQuote({ status: newStatus });
    
    // Send email notification when status changes to "quoted" with price
    if (newStatus === "quoted" && price && !isNaN(parseFloat(price))) {
      try {
        await fetch("/api/send-customer-email", {
          ...
          body: JSON.stringify({
            to: customerEmail,
            toName: customerName,
            subject: "Your Quote is Ready - PlusPro Services",
            message: `Dear ${customerName},\n\nYour quote is ready!\n\nQuote Amount: $${parseFloat(price).toFixed(2)}\n\nPlease contact us to proceed with this quote.\n\nBest regards,\nPlusPro Services Team`,
            type: "quote_ready",
          }),
        });
      } catch (error) {
  ...
  const handlePriceChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setTimeout(async () => {
      if (newPrice && !isNaN(parseFloat(newPrice))) {
        await updateQuote({ quote_price: parseFloat(newPrice) });
      }
    }, 1000);
  };
  ...
  const handleAssignStaff = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    await updateQuote({ assigned_to: staffId || null });
  ...
  const updateQuote = async (updates: any) => {
    setUpdating(true);
    setSaveStatus(null);
  ...
  const handleCancel = async () => {
    ...
    await updateQuote({ status: "cancelled" });
    ...
    try {
      await fetch("/api/send-customer-email", {
        ...
        body: JSON.stringify({
          to: customerEmail,
          toName: customerName,
          subject: "Quote Request Cancelled - PlusPro Services",
          message: `Dear ${customerName},\n\nWe regret to inform you that your quote request has been cancelled. If you have any questions, please contact us.\n\nBest regards,\nPlusPro Services Team`,
          ...
          <div className="space-y-4 py-4">
            <div>
              ...
              <Input
                ...
                placeholder="e.g., Your Quote is Ready"
                required
              />
            </div>
            <div>
